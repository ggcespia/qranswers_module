...
const Api = {
	...
	async getApiKeyFromDbKey(baseId, dbKey) {
		const _savedApiKey = this._apiKey;
		const _savedApiBase = this._apiBase;

		this._apiKey = dbKey;
		this._apiBase = 'https://api.qr-answers.com/v1';
		const url = `${this._apiBase}/apikey/get/${baseId}/${dbKey}`;
		const myRes =await this.doFetch(url)
		this._apiKey = _savedApiKey;
		this._apiBase = _savedApiBase;
		return myRes;
	},
